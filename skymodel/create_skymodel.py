...
def create_full_gleam_skymodel(table, metafits, outname=None, alpha=None,
                               threshold=1., ref_threshold=0.1, exclude_coords=None,
                               exclusion_zone=1., radius=180., nmax=200, 
                               ref_key="S_200", ref_freq=200.315, 
                               ignore_magellanic=True, index_limits=[-3., 2.],
                               ):
    """
    """
    ...
    coords = SkyCoord(ra=catalogue["RAJ2000"], dec=catalogue["DEJ2000"], 
                      unit=(u.deg, u.deg))
    seps = pnt.separation(coords).value
    catalogue = catalogue[seps < radius]
    logging.info("Sources after radial cut: {}".format(len(catalogue))) 
    ...
    stokesI = beam_value(catalogue["RAJ2000"], catalogue["DEJ2000"], t, delays, 
                         freq*1.e6, return_I=True)
    ...
    for i in range(len(catalogue)):
        ...
        at_flux[i] = fitting.cpowerlaw_from_ref(freq, ref_freq, row[ref_key], 
                                                row["alpha"], row["beta"])
...
# 
    ...
    with open(outname, "w+") as o:
        ...
        for i in range(len(catalogue)):
            ...
            flux = []
            for f in FREQ_LIST:
                flux.append(fitting.cpowerlaw_from_ref(f, ref_freq, catalogue[i][ref_key], 
                                                catalogue[i]["alpha"], catalogue[i]["beta"]))
